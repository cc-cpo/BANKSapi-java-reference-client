...
public final class AroundRequestHandler implements IHTTPClient {
    ...
    private <T> void debugResponse(Response<T> response) {
        if (filter == null) {
            return;
        }
        final Instant responseTimestamp = Instant.now();

        RequestDetails requestDebugDetails = new RequestDetails(HttpMethod.PUT, wrapped.getRequestUrl(), null, correlationId);
        try {
